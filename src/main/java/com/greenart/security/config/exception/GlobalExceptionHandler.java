...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    // @Valid 또는 @Validated 로 검증된 객체에서 유효성 검사가 실패할 때 발생하는 예외를 처리.
    @Override
    ...
    // JWT 관련 예외인 MalformedJwtException 이나 SignatureException 이 발생했을 때 처리. ( 토큰 값이 유효하지 않을 때, 토큰이 오염되었을 때 )
...
//        return handleExceptionInternal(UserErrorCode.INVALID_TOKEN);
        log.info("MalformedJwtException - handlerException");
...
//        return handleExceptionInternal(UserErrorCode.EXPIRED_TOKEN);
        log.info("ExpiredJwtException - handlerException");
        return handleExceptionInternal(CommonErrorCode.INTERNAL_SERVER_ERROR);
    }
